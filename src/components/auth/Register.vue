...
<!--            <div class="logo-placeholder"></div>-->
          ...
          <h1 class="main-title">注册</h1>
          <p class="description" style="margin-bottom: 20px;">创建一个新账户。</p>
          ...
          <form @submit.prevent="register">
            ...
            <button
              ...
              :disabled="loading"
            >
              <span class="button-text">{{ loading ? '处理中...' : '注册' }}</span>
            ...
            <div style="margin-top: 20px; text-align: center;">
              <router-link to="/login" class="green-text">已有账号？去登录</router-link>
            </div>
          </form>
        </div>
      </div>
  ...
  setup() {
    ...
    const register = async () => {
      if (registerForm.password !== registerForm.confirm_password) {
        store.commit('auth/SET_ERROR', '两次输入的密码不一致')
        return
      }
